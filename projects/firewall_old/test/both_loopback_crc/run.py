...
for i in range(NUM_PKTS):
    ...
    for port in range(4):
        ...
        #                     pkt_len=random.randint(60,1514))/hdr)
        #pkts[port].append(scapy.Ether(dst=DA, src=SA)/scapy.IP(dst=DST_IP, src=SRC_IP,ttl=TTL,len=random.randint(60,1514))/hdr)
        load = ''
        lengthh = random.randint(60,1514)
        for genr in range(lengthh):
           load += chr(randint(0,255))
        pkts[port].append(scapy.Ether(dst=DA, src=SA)/scapy.IP(dst=DST_IP, src=SRC_IP,ttl=TTL)/hdr/load)
...
for i in range(4):
    print "MAC_GRP_0_RX_PKTS_STORED: ", reg_defines.MAC_GRP_0_RX_QUEUE_NUM_PKTS_STORED_REG()
    nftest_regread_expect(reg_defines.MAC_GRP_0_RX_QUEUE_NUM_PKTS_STORED_REG() + i*0x40000, NUM_PKTS)
    print "MAC_GRP_0_PKTS_SENT: ", reg_defines.MAC_GRP_0_TX_QUEUE_NUM_PKTS_SENT_REG()
    nftest_regread_expect(reg_defines.MAC_GRP_0_TX_QUEUE_NUM_PKTS_SENT_REG() + i*0x40000, NUM_PKTS)
    print "MAC_GRP_0_PKTS_PUSHED: ", reg_defines.MAC_GRP_0_RX_QUEUE_NUM_BYTES_PUSHED_REG()
    nftest_regread_expect(reg_defines.MAC_GRP_0_RX_QUEUE_NUM_BYTES_PUSHED_REG() + i*0x40000, totalPktLengths[i])
    print "MAC_GRP_0_TX_PUSHED: ", reg_defines.MAC_GRP_0_TX_QUEUE_NUM_BYTES_PUSHED_REG()
